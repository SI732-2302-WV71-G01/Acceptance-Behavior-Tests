Feature: Revisar productos del carrito de compra de la aplicación
  ...
  #Happy Path
  Scenario: El usuario experto que ya inició sesión visualiza correctamente
  ...
  #Sad Path
  Scenario: El usuario experto añade productos de su tienda al carrito de compras antes de iniciar sesión
    ...
    #Alternative path
  Scenario:El usuario revisa el carrito de compras después de haber cerrado sesión
    Given Hace click al botón de Access
    And Rellena los todos los campos solicitados con datos existentes de cuenta de experto
    And Hace click en Sign In
    And el usuario es redirigido a la página principal de la webapp
